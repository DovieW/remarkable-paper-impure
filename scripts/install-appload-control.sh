...
readonly checksum="$root/build/appload-control/appload.so.sha256"
...
test -f "$checksum" || { echo "adapter checksum is missing" >&2; exit 1; }
sha256sum -c "$checksum"
file "$artifact" | grep -q 'ELF 64-bit.*ARM aarch64' || {
  echo "adapter is not an aarch64 ELF shared object" >&2
  exit 1
}
if $dry_run; then
...
ssh -- "$host" "set -eu
  mkdir -p /home/root/.local/share/paperboard/backups
  chmod 0700 /home/root/.local/share/paperboard/backups
  cp '$remote' '/home/root/.local/share/paperboard/backups/appload.so.$stamp'
  cp /tmp/paperboard-appload.so '$remote'
  chmod 0755 '$remote'
