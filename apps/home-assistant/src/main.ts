...
const session = required(process.env.CANVAS_SESSION, "CANVAS_SESSION");
...
for (;;) {
  const response = await relay.canvasEvents(device, session, cursor) as { events?: Array<{ id: string; cursor: number; action_id: string; value: unknown }> };
  for (const event of response.events ?? []) {
    ...
    await relay.acknowledgeCanvasEvent(device, session, event.id);
  }
